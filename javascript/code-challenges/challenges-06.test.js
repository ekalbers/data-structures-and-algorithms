...
const getNames = (arr) => arr.map(item => [...item.name].reduce((accum, current) => accum = current + accum));
...
const setStatusAsAuthor = (people) => people.map(item => item.isAuthor = true);
...
const append = (arr1, arr2) => arr2.forEach(item => arr1.push(item));
