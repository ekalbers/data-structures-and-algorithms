...
const screenForNames = (arr) => arr.filter(item => /^(Mr\. |Mrs\. |Ms\. |Dr\. )[a-zA-Z][a-zA-Z ]+$/.test(item));
...
let biggerThanLuke = (arr) => {
  let masses = arr.map(item => item['mass']);
  let names = arr.map(item => item['name']);
  let bigNames = [];
  for (let x = 1; x < arr.length; x++) {
    if (parseInt(masses[x]) > parseInt(masses[0])) {
      bigNames.push(names[x]);
    }
  }
  return bigNames.join(' - ');
};
...
const detectTicTacToeWin = (board) => {
  let possible = [[board[0][0], board[0][1], board[0][2]], [board[0][0], board[1][0], board[2][0]], [board[0][0], board[1][1], board[2][2]], [board[0][1], board[1][1], board[2][1]], [board[0][2], board[1][2], board[2][2]], [board[1][0], board[1][1], board[1][2]], [board[2][0], board[2][1], board[2][2]]];
  let win = false;
