...
const findHappiness = (arr) => arr.filter(item => /[:]\)/.test(item));
...
const standardizePhoneNumbers = (arr) => arr.map(item => [...item.matchAll(/\d+/g)].join(''));
...
const onlyOddChars = (str) => str.split('').reduce((accum, current, index) => index % 2 === 1 ? accum + current : accum, '');
