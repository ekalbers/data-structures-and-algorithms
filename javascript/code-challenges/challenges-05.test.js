...
const addValues = (arr) => arr.reduce((accum, current) => accum + current, 0);
...
const addPurchases = (arr) => arr.reduce((accum, current) => accum + current.purchasePrice, 0);
...
const countNumberOfElements = (arr) => arr.reduce((accum) => accum + 1, 0);
...
const returnNames = (arr) => arr.map(item => item.name);
...
const reversedString = (str) => [...str].reduce((accum, current) => accum = current + accum);
...
const calculateAverage = (arr) => addValues(arr) / arr.length;
...
const extractStats = (snorlaxData) => {
  /* let arr = [];
  arr = snorlaxData.stats.reduce((accum, current) => {
    let statName = current.stat['name'];
    let baseStat = current.baseStat;
    { statName: baseStat });
}, []);
return arr; */
};
