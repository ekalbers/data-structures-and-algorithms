...
E.g. [4,2,7,5,9,2] -> 9
------------------------------------------------------------------------------------------------ */
const maxInArray = (arr) => arr.reduce((accum, current) => accum < current ? accum = current : accum);
...
const checkValues = (obj, value) => Object.values(obj).indexOf(value) > -1 ? true : false;
...
const hasChildrenValues = (arr, character) => {
  ...
  arr.forEach(item => {
    ...
    if (values[0] === character && values.length > 3) {
      x = true;
    }
  });
  return x;
