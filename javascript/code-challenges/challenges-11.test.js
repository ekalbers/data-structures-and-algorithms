...
function transformToLis(obj) {
  ...
  for (let x = 0; x < keys.length; x++) {
    keysValues.push(`<li>${keys[x]}: ${values[x]}</li>`);
  }
  return keysValues;
}
...
const count = (target, input) => {
  let counter = 0;
  input.forEach(item => item.forEach(item => item === target ? counter++ : 0));
  return counter;
};
...
const totalSum = (input) => {
  let total = 0;
  input.forEach(item => item.forEach(item => total += item));
  return total;
};
...
const divisibleByFiveTwoToThePower = (input) => input.map(item => item.filter(value => value % 5 === 0 && typeof value === 'number').map(item => 2 ** item));
...
let findMaleAndFemale = (data) => {
  let maleFemale = data.filter(value => value.gender === 'male' || value.gender === 'female');
  let names = maleFemale.map(item => item.name);
  let str = '';
...
let findShortest = (data) => data.reduce((accum, current) => accum.height < current.height ? accum = current : accum).name;
